...
IF FDATE IS NOT NULL AND TDATE IS NOT NULL THEN
	SET NDays = DATEDIFF(TDATE, FDATE);
...
ELSE
	...
	CREATE TABLE temp_count_clients
	SELECT DISTINCT /*+PARALLEL(32)*/ id as cbs_party_id from temp_clients
	WHERE id IN (SELECT cbs_party_id from temp_clients_count where Flag = 'CD');

	INSERT IGNORE INTO temp_count_clients
	SELECT DISTINCT /*+PARALLEL(32)*/ id as cbs_party_id from temp_clients
	WHERE id IN (SELECT cbs_party_id from temp_clients_count where Flag = 'LD');
END IF;
...
DROP TABLE IF EXISTS `ODS_clients__addresses`;
CREATE TABLE `ODS_clients__addresses` (
...
DROP TABLE IF EXISTS `ODS_clients__custom_fields`;
CREATE TABLE `ODS_clients__custom_fields` (
...
DROP TABLE IF EXISTS `ODS_clients__id_documents`;
CREATE TABLE `ODS_clients__id_documents` (
...
DROP TABLE IF EXISTS `ODS_mig_clients`;
CREATE TABLE `ODS_mig_clients` (
...
DROP TABLE IF EXISTS `ODS_party`;
CREATE TABLE `ODS_party` (
...
DROP TABLE IF EXISTS `ODS_address`;
...
DROP TABLE IF EXISTS `ODS_consent_details`;
CREATE TABLE `ODS_consent_details` (
...
DROP TABLE IF EXISTS `ODS_contact`;
CREATE TABLE `ODS_contact` (
...
DROP TABLE IF EXISTS `ODS_party_status`;
CREATE TABLE `ODS_party_status` (
...
DROP TABLE IF EXISTS `ODS_relationship`;
CREATE TABLE `ODS_relationship` (
...
DROP TABLE IF EXISTS `ODS_system_integration_status`;
CREATE TABLE `ODS_system_integration_status` (
...
DROP TABLE IF EXISTS `ODS_occupation`;
CREATE TABLE `ODS_occupation` (
...
DROP TABLE IF EXISTS `ODS_kyc_details`;
CREATE TABLE `ODS_kyc_details` (
...
DROP TABLE IF EXISTS `ODS_asset_details`;
CREATE TABLE `ODS_asset_details` (
...
DROP TABLE IF EXISTS `ODS_device_details`;
CREATE TABLE `ODS_device_details` (
...
DROP TABLE IF EXISTS `ODS_external_system_response`;
CREATE TABLE `ODS_external_system_response` (
  `Id` bigint NOT NULL AUTO_INCREMENT,
  `Party_Id` bigint(10) unsigned zerofill NOT NULL,
...
DROP TABLE IF EXISTS `ODS_referral_promotion`;
CREATE TABLE `ODS_referral_promotion` (
  `Id` bigint NOT NULL AUTO_INCREMENT,
  `Party_Id` bigint(10) unsigned zerofill NOT NULL,
...
/****** ODS_Reconciliation_Count ******/

CREATE TABLE IF NOT EXISTS `ods_recon_count` (
  `reconciliation_date` datetime DEFAULT CURRENT_TIMESTAMP,
  `batch_name` varchar(50) DEFAULT NULL,
  `ods_mig_cllients` bigint DEFAULT NULL,
  `ods_party` bigint DEFAULT NULL,
  `ods_party_status` bigint DEFAULT NULL,
  `ods_address` bigint DEFAULT NULL,
  `ods_consent_details` bigint DEFAULT NULL,
  `ods_contact` bigint DEFAULT NULL,
  `ods_relationship` bigint DEFAULT NULL,
  `ods_system_integration_status` bigint DEFAULT NULL,
  `ods_occupation` bigint DEFAULT NULL,
  `ods_kyc_details` bigint DEFAULT NULL,
  `ods_asset_details` bigint DEFAULT NULL,
  `ods_device_details` bigint DEFAULT NULL,
  `ods_external_system_response` bigint DEFAULT NULL,
  `ods_referral_promotion` bigint DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
SELECT /*+PARALLEL(32) USE_HASH(4)*/ b.party_id AS Party_Id,
	...
	FROM ods_clients a INNER JOIN
	party b ON b.cbs_party_id = a.id;
...
CALL insert_transaction("ODS_party", "INSERT INTO ODS_party (party_id, cbs_party_id, first_name, last_name, middle_name, dob, gender, salutation, suffix, civil_status, place_of_birth, nationality, no_of_dependents, arn_no, idm_arn_no, party_status, is_dosri, is_rpt, is_fatca, party_type, preferred_address_id, fatca_W9_IdType, fatca_W9_IdNumber, app_channel_type, app_product_type, app_party_type, channel_type, product_type, sssgsis, tinid, is_record_edited, party_matrix_header_id, My_Job, Card_Issuance_Status, is_active, created_date, created_by, approved_date, activation_date, Is_Latest_rec) 
...
CALL insert_transaction("ODS_address (CA)", "INSERT INTO ODS_address (`Party_Id`, `Address_Type`, `Number_And_Street`, `Barangay`, `LandMark`, `Comments`, `City`, `State`, `Region`, `Country`, `Country_Code`, `Zip_Code`, `Nom_Index`, `Date_Since_Residing`, `Is_Preferred_Address`, `Province`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
...
CALL insert_transaction("ODS_address (PA)", "INSERT INTO ODS_address (`Party_Id`, `Address_Type`, `Number_And_Street`, `Barangay`, `LandMark`, `Comments`, `City`, `State`, `Region`, `Country`, `Country_Code`, `Zip_Code`, `Nom_Index`, `Date_Since_Residing`, `Is_Preferred_Address`, `Province`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
...
CALL insert_transaction("ODS_address (BA)", "INSERT INTO ODS_address (`Party_Id`, `Address_Type`, `Number_And_Street`, `Barangay`, `LandMark`, `Comments`, `City`, `State`, `Region`, `Country`, `Country_Code`, `Zip_Code`, `Nom_Index`, `Date_Since_Residing`, `Is_Preferred_Address`, `Province`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with clients__addresses as (select /*+PARALLEL(8)*/ distinct a.party_id As Party_Id, 
...
CALL insert_transaction("ODS_address (EA)", "INSERT INTO ODS_address (`Party_Id`, `Address_Type`, `Number_And_Street`, `Barangay`, `LandMark`, `Comments`, `City`, `State`, `Region`, `Country`, `Country_Code`, `Zip_Code`, `Nom_Index`, `Date_Since_Residing`, `Is_Preferred_Address`, `Province`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with clients__addresses as (select /*+PARALLEL(8)*/ distinct a.party_id As Party_Id, 
...
CALL insert_transaction("ODS_Consent_Details", "INSERT INTO ODS_Consent_Details (Party_Id, Consent_Type, Consent_Capture_Source, Consent_Channel, Status, Comments, Consent_Details, Created_Date, Created_By, Is_Latest_rec)
...
Select distinct /*+PARALLEL(8)*/ a.party_id as party_id, 'Preferred Email' as Contact_Type, b.email_address as
 Contact_Value, NULL as Contact_Period, 1 as Is_Preferred, 1 as Is_Active, current_date as Created_Date,
 CONCAT('MIG_',CURRENT_DATE) as Created_By, 'Y' as Is_Latest_rec From ods_party a
 INNER JOIN ods_clients b ON a.cbs_party_id = b.id INNER JOIN temp_clients_count j on j.cbs_party_id=b.id where b.email_address <> '';
 
Create table contact_mobile_phone_temp 
select distinct /*+PARALLEL(8)*/ c.party_id as party_id, 'Mobile Phone Number'
 as Contact_Type, d.mobile_phone as Contact_Value, NULL as Contact_Period, 
 1 as Is_Preferred, 1 as Is_Active, current_date as Created_Date, CONCAT('MIG_',CURRENT_DATE) 
 as Created_By, 'Y' as Is_Latest_rec From ods_party c INNER JOIN ods_clients d 
 ON c.cbs_party_id = d.id INNER JOIN temp_clients_count j on j.cbs_party_id=d.id where d.mobile_phone <> '';
 
Create table contact_alternate_phone_temp 
select distinct /*+PARALLEL(8)*/ a.party_id as party_id, 'Alternate Mobile Number' as Contact_Type,
 b.home_phone as Contact_Value, NULL as Contact_Period, 1 as Is_Preferred, 
 1 as Is_Active, current_date as Created_Date, CONCAT('MIG_',CURRENT_DATE) as Created_By, 'Y' as Is_Latest_rec
 From ods_party a INNER JOIN ods_clients b ON a.cbs_party_id = b.id INNER JOIN temp_clients_count j on j.cbs_party_id=b.id where b.home_phone <> '';
 	   
CALL insert_transaction("ODS_contact", "INSERT INTO `ODS_contact` (`Party_Id`, `Contact_Type`, `Contact_Value`, `Contact_Period`, `Is_Preferred`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
...
CALL insert_transaction("ODS_party_status", "INSERT INTO `ODS_party_status` (`party_id`, `cbs_party_id`, `status_type`, `status_flag`, `created_date`, `created_by`, `Is_Latest_rec`)
...
INSERT INTO ODS_relationship (`Party_Id`, `Relationship_Type`, `Name`, `DOB`, `Nationality`, `Address`,
`Share_Percentage`, `Is_Nominee`, `Nominee_Index`, `Is_Active`, `created_date`, `created_by`, `Is_Latest_rec`)
with cte1 as (SELECT /*+PARALLEL(8)*/ c.party_id as party_id,
...
CALL insert_transaction("ODS_relationship (SD)", "INSERT INTO ODS_relationship (`Party_Id`, `Relationship_Type`, `Name`, `DOB`, `Nationality`, `Address`,
`Share_Percentage`, `Is_Nominee`, `Nominee_Index`, `Is_Active`, `created_date`, `created_by`, `Is_Latest_rec`)
with cte1 as (SELECT /*+PARALLEL(8)*/ c.party_id AS party_id, b.id AS cbs_party_id, a.id, _sdc_source_key_id, _sdc_level_0_id, value,
...
CALL insert_transaction("ODS_system_integration_status", "INSERT INTO `ODS_system_integration_status` (`party_id`, `system_name`, `enrollment_status`, `Is_Active`, `created_date`, `created_by`, `Is_Latest_rec`)
SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
                c.id AS System_Name,
...
SELECT DISTINCT a.party_id AS Party_Id,
...
CALL insert_transaction("ODS_kyc_details", "INSERT INTO `ODS_kyc_details` (`Party_Id`, `Document_Id`, `Document_Type`, `Issuing_Authority`, `Valid_Until`, `National_Id`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
...
CALL insert_transaction("ODS_asset_details (FOH)", "INSERT INTO ODS_asset_details (`Party_Id`, `Asset_Number`, `Asset_Description`, `Product_Id`, `Purchase_Date`, `Status`, `Quantity`, `Price`, `Valid_Upto`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
CALL insert_transaction("ODS_asset_details (FOC)", "INSERT INTO ODS_asset_details (`Party_Id`, `Asset_Number`, `Asset_Description`, `Product_Id`, `Purchase_Date`, `Status`, `Quantity`, `Price`, `Valid_Upto`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
CALL insert_transaction("ODS_device_details", "INSERT INTO ODS_device_details (`Party_Id`, `Device_IP`, `Latitude`, `Longitude`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
INSERT INTO ODS_external_system_response (`Party_Id`, `External_System_Name`, `External_Response_Details`, `Overall_Status`, `Case_Id`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with zoloz_cte as (SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
INSERT INTO ODS_external_system_response (`Party_Id`, `External_System_Name`, `External_Response_Details`, `Overall_Status`, `Case_Id`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with AML_cte as (SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
INSERT INTO ODS_external_system_response (`Party_Id`, `External_System_Name`, `External_Response_Details`, `Overall_Status`, `Case_Id`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with IdMission_cte as (SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
INSERT INTO ODS_referral_promotion (`Party_Id`, `Referral_Code`, `Promo_Code`, `Company_Code`, `Agent_Code`, `Ref_Promo_Flag`, `Is_Active`, `Created_Date`, `Created_By`, `Is_Latest_rec`)
with Referral_Promotion_cte as (SELECT /*+PARALLEL(8)*/ DISTINCT a.party_id AS Party_Id,
...
SELECT DISTINCT /*+PARALLEL(32)*/ a.party_id,a.cbs_party_id,
		...
		IFNULL(CONCAT(UPPER(SUBSTRING(Party_Status,1,1)),LOWER(SUBSTRING(Party_Status,2))), 0),
		IFNULL(Is_FATCA,0),
		IFNULL(Party_Type,0),
...
FROM ods_party_status a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_address a INNER JOIN ods_party b ON b.party_id= a.party_id;
...
FROM ods_consent_details a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_contact a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_relationship a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_system_integration_status a INNER JOIN ods_party b ON b.party_id = a.party_id;

DROP TEMPORARY TABLE IF EXISTS chksum_ods_occupation;
CREATE TEMPORARY TABLE chksum_ods_occupation 	   
SELECT DISTINCT /*+PARALLEL(32)*/ a.party_id, b.cbs_party_id,nature_of_occupation, nature_of_occupation_code, designation,
	   ...
	   "cbs_party_id, nature_of_occupation, nature_of_occupation_code, designation,
       organization, industry_type, industry_type_code, occupation_type, occupation_type_code,
 	   occupation_status, occupation_status_code,date_of_commencement, monthly_income, annual_income,
	   fund_source_name, fund_source_code,source_of_wealth, proof_of_sow, party_linked_companies,
	   party_banks, proof_of_address,proof_of_income, salary_period, salary_dates,
	   proof_of_billing, profession"as Taged_columns  	   
FROM ods_occupation a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_kyc_details a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_asset_details a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_device_details a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_external_system_response a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
FROM ods_referral_promotion a INNER JOIN ods_party b ON b.party_id = a.party_id;
...
SELECT DISTINCT /*+PARALLEL(32)*/ a.party_id,a.cbs_party_id,
		...
		"First_Name, Last_Name, Middle_Name, DOB, Gender, Salutation, Suffix, Civil_Status, Place_Of_Birth, Nationality, No_Of_Dependents, Arn_No, Idm_Arn_No, Party_Status, Is_FATCA, Party_Type, fatca_W9_IdType, fatca_W9_IdNumber, sssGsis, tinId, Card_Issuance_Status, My_Job, Approved_Date, Activation_Date"
		 as Taged_columns
FROM party a WHERE a.is_active = 1 AND a.is_latest_rec ='Y' AND SUBSTRING(a.Created_By,1,4)='MIG_' AND a.Approved_Date IS NOT NULL
AND EXISTS (SELECT 1 FROM temp_count_clients b WHERE b.cbs_party_id = a.cbs_party_id);
...
WHERE a.is_active = 1 AND a.is_latest_rec ='Y' AND SUBSTRING(a.Created_By,1,4)='MIG_' 
AND EXISTS (SELECT 1 FROM temp_count_clients c WHERE c.cbs_party_id = b.cbs_party_id);
...
IFNULL(Enrollment_Status,0))) as Hashtag_val, 
"System_Name,Enrollment_Status,Status" as Taged_columns  
FROM system_integration_status a INNER JOIN party b ON a.party_id = b.party_id
WHERE a.is_latest_rec = 'Y' AND SUBSTRING(a.Created_By,1,4)='MIG_' 
-- WHERE a.is_active = 1 AND a.is_latest_rec = 'Y' 
AND EXISTS (SELECT 1 FROM temp_count_clients c WHERE c.cbs_party_id = b.cbs_party_id);

DROP TEMPORARY TABLE IF EXISTS temp_chksum_crm_occupation;
CREATE TEMPORARY TABLE temp_chksum_crm_occupation
SELECT DISTINCT(a.party_id) as party_id, a.cbs_party_id FROM party a INNER JOIN temp_clients_count b ON b.cbs_party_id = a.cbs_party_id
WHERE a.is_active = 1 AND a.is_latest_rec = 'Y' AND SUBSTRING(a.Created_By,1,4)='MIG_';
...
SELECT DISTINCT /*+PARALLEL(32)*/ a.party_id, b.cbs_party_id,nature_of_occupation, nature_of_occupation_code, designation,
	   ...
	   party_banks, proof_of_address,proof_of_income, salary_period, salary_dates,
	   proof_of_billing, profession"as Taged_columns  	   
FROM occupation a INNER JOIN temp_chksum_crm_occupation b ON a.party_id = b.party_id
WHERE a.is_active = 1 AND a.is_latest_rec = 'Y' AND SUBSTRING(a.Created_By,1,4)='MIG_' ;
...
CREATE TABLE `Checksum_CRM_ODS`  ( 
     ...
     `cbs_party_id`       VARCHAR(20) NOT NULL,
	 ...
	 KEY `idx_ccods_cbs_party_id` (`cbs_party_id`)
  ) engine=innodb DEFAULT charset=utf8mb4 COLLATE=utf8mb4_0900_ai_ci; 
...
Insert into Checksum_CRM_ODS (`Source`, `Table_Name`, `CBS_party_id`, `Taged_columns`, `Source_Hashtag_val`, `Target_Hashtag_val`, `MIG_Status`)
Select  Distinct 'CRM_ODS', 'referral_promotion' as Table_Name, a.CBS_party_id,a.Taged_columns, a.Hashtag_val, b.Hashtag_val,
case when a.Hashtag_val=b.Hashtag_val Then 'Matched' else 'Not Macthed' End as MIG_Status
...
from address a INNER JOIN party b ON a.party_id = b.party_id 
where a.is_active=1 AND a.is_latest_rec = 'Y' and SUBSTRING(a.Created_By,1,4)='MIG_' and b.Approved_Date IS NOT NULL
and b.cbs_party_id in(select distinct cbs_party_id from checksum_crm_ods)),
...
country,country_code, zip_code,nom_index,date_since_residing,is_preferred_address,province 
from ODS_address a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
...
party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'contact'as CRM_TABLE,'ODS' as CRM, b.cbs_party_id,contact_type ,contact_value ,contact_period ,is_preferred
 from ODS_contact a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'party_status'as CRM_TABLE,'ODS' as CRM, b.cbs_party_id,status_type, status_flag,a.is_latest_rec
 from ODS_party_status a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'relationship'as CRM_TABLE,'ODS' as CRM, b.cbs_party_id,relationship_type, name,a.dob,a.nationality, address, share_percentage, is_nominee,nominee_index
 from ODS_relationship a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'system_integration_status'as CRM_TABLE,'ODS' as CRM, cbs_party_id,system_name,enrollment_status
 from ODS_system_integration_status a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
industry_type_code, occupation_type, occupation_type_code, designation, occupation_status,
...
party_linked_companies, party_banks, proof_of_address, proof_of_income,
salary_period, salary_dates, proof_of_billing, profession
 from ODS_occupation a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
UNION
select  a.CRM_TABLE,'designation',a.cbs_party_id,a.designation as 'CRM_designation',b.designation as 'ODS_designation'
from cte1  a inner join cte2 b on a.cbs_party_id=b.cbs_party_id where a.occupation_type=b.occupation_type and upper(trim(a.designation))<>upper(trim(b.designation))
UNION
...
cte2 as (select distinct 'kyc_details'as CRM_TABLE,'ODS' as CRM,cbs_party_id, document_id,
		document_type, issuing_authority, valid_until, national_id
 from ODS_kyc_details a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'asset_details'as CRM_TABLE,'ODS' as CRM,cbs_party_id, asset_number,
	asset_description, product_id, purchase_date, status, quantity, price,valid_upto
 from ODS_asset_details a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'device_details'as CRM_TABLE,'ODS' as CRM,cbs_party_id, device_ip, latitude,longitude
 from ODS_device_details a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
external_response_details, overall_status, case_id
 from ODS_external_system_response a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
cte2 as (select distinct 'referral_promotion'as CRM_TABLE,'ODS' as CRM,cbs_party_id,referral_code, promo_code, company_code, agent_code, ref_promo_flag
 from ODS_referral_promotion a INNER JOIN party b ON a.party_id = b.party_id where b.cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
INSERT INTO crm_final_rcon_tab_temp(CRM_TABLE, column_names, cbs_party_id, crm_column_data, ods_column_date)
WITH cte1 AS
(select distinct 'Party'as CRM_TABLE,'CRM' as CRM,cbs_party_id,First_Name, Last_Name, Middle_Name, DOB, Gender, Salutation, Suffix, Civil_Status, Place_Of_Birth, Nationality, No_Of_Dependents, Arn_No, Idm_Arn_No, UPPER(Party_Status) as Party_Status, Is_FATCA, Party_Type, fatca_W9_IdType, fatca_W9_IdNumber, sssGsis, tinId, Card_Issuance_Status, My_Job, Approved_Date, Activation_Date from party 
where is_active= 1 AND is_latest_rec = 'Y' and SUBSTRING(Created_By,1,4)='MIG_' and Approved_Date IS NOT NULL
and cbs_party_id in(select distinct cbs_party_id from checksum_crm_ods)),
cte2 as (select distinct 'Party'as CRM_TABLE,'ODS' as CRM,cbs_party_id,First_Name, Last_Name, Middle_Name, DOB, Gender, Salutation, Suffix, Civil_Status, Place_Of_Birth, Nationality, No_Of_Dependents, Arn_No, Idm_Arn_No, UPPER(Party_Status) as Party_Status, Is_FATCA, Party_Type, fatca_W9_IdType, fatca_W9_IdNumber, sssGsis, tinId, Card_Issuance_Status, My_Job, Approved_Date, Activation_Date from ods_party where cbs_party_id 
in(select distinct cbs_party_id from checksum_crm_ods))
select * from(
...
INSERT INTO ods_recon_count
SELECT CURRENT_TIMESTAMP AS reconciliation_date,
'Reconciliation of Data' AS batch_name,
